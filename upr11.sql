...
delimiter |
CREATE TRIGGER before_salarypayment_update BEFORE DELETE ON salarypayments
FOR EACH ROW 
INSERT INTO salarypayments_log (coach_id, month, year, salaryAmount, dateOfPayment, deleted_at)
VALUES (OLD.coach_id, OLD.month, OLD.year, OLD.salaryAmount, OLD.dateOfPayment, 
NOW());

DELETE FROM salarypayments;

INSERT INTO salarypayments (coach_id, month, year, salaryAmount, dateOfPayment)
SELECT coach_id, month, year, salaryAmount, dateOfPayment
FROM salarypayments_log;


CREATE TRIGGER students_sportgroups_check BEFORE INSERT ON student_sport FOR EACH ROW
BEGIN
    IF (SELECT COUNT(*) 
		FROM student_sport 
		WHERE student_id = NEW.student_id) >= 2 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'This student is already in 2 sport groups';
    END IF;
END;

DROP VIEW IF EXISTS student_groups_view;
CREATE VIEW student_groups_view as
SELECT students.name,
count(student_sport.student_id)
from students
join student_sport on students.id=student_sport.student_id
Group by students.name;

--Напишетепроцедура, с която по подадено име на спортсе извеждат имената на треньорите,
 които водят съответните групи, мястото, часът и денят на тренировка.
